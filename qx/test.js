...
[rewrite_local]
#修改
^https?:\/\/api\.revenuecat\.com\/v1\/(subscribers\/[^\/]+$|receipts$) url script-response-body https://raw.githubusercontent.com/Mikephie/Script/main/qx/test.js
#清理
^https?:\/\/api\.revenuecat\.com\/v1\/(subscribers\/[^\/]+$|receipts$) url script-request-header https://raw.githubusercontent.com/Mikephie/Script/main/qx/test.js
...
const Q = {};
const mike = JSON.parse(typeof $response != "undefined" && $response.body || null);
if (typeof $response == "undefined") {
  delete $request.headers["x-revenuecat-etag"];
  delete $request.headers["X-RevenueCat-ETag"];
  Q.headers = $request.headers;
} else if (mike && mike.subscriber) {
  mike.subscriber.subscriptions = mike.subscriber.subscriptions || {};
  mike.subscriber.entitlements = mike.subscriber.entitlements || {};
  var headers = {};
  ...
  for (const i in UAMappings) {
    if (new RegExp(`^${i}`, 'i').test(UA)) {
      ...
      mike.subscriber.subscriptions = {};
      mike.subscriber.subscriptions[id] = data;
      mike.subscriber.entitlements[name] = JSON.parse(JSON.stringify(data));
      mike.subscriber.entitlements[name].product_identifier = id;
      break;
    }
  }
  Q.body = JSON.stringify(mike);
}
$done(Q);