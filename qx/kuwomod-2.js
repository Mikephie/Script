...
if (url.indexOf(KuWo_Enc) != -0x1) {
    !(async () => {
        let b = new URL(url).searchParams;
        let c = b.get('uid');
        if ('number' !== typeof c) c = url.replace(/.*?uid=(\d+).*/, '$1');
...
if (url.match(KuWo_BookPageAD)) {
    ...
    while (obj.data.child[i]) {
        if (/^小焦点/ ['test'](obj.data.child[i].label)) delete obj.data.child[i].child;
        i++;
    }
    body = $.toStr(obj);
    const KuWoLp = {};
...
if (url.indexOf(KuWo_TabAD) != -0x1) {
    if ('undefined' !== typeof obj.data.tab.vipTypes[0x0]) {
        let i = 0x1;
        while (obj.data.tab.vipTypes[0x0].topics[i]) {
            delete obj.data.tab.vipTypes[0x0].topics[i];
            i++;
        }
    }
    body = $.toStr(obj);
...
async function getInfo(c, d) {
    ...
    if (!KuWo.user || c != KuWo.user || !KuWo.endTime || new Date().getTime() > KuWo.endTime || !KuWo.keys) {
        $.log('正在获取 ' + c + ' 的授权信息…');
        const f = {};
        f.url = 'https://napi.ltd/getInfo';
        f.body = e;
        let g = $.toObj(await $.http.post(f).then(h => h.body));
        for (let h in g) {
            if (g.hasOwnProperty(h)) {
                if ('hiJNb' === 'CzwMt') {
                    var k = string.charCodeAt(n);
                    if (k < 0x80) {
                        utftext += String.fromCharCode(k);
                    } else if (k > 0x7f && k < 0x800) {
                        utftext += String.fromCharCode(k >> 0x6 | 0xc0);
                        utftext += String.fromCharCode(k & 0x3f | 0x80);
                    } else {
                        utftext += String.fromCharCode(k >> 0xc | 0xe0);
                        utftext += String.fromCharCode(k >> 0x6 & 0x3f | 0x80);
                        utftext += String.fromCharCode(k & 0x3f | 0x80);
                    }
                } else {
                    KuWo[h] = g[h];
        ...
        if (g.isVip) {
            ...
            const n = {};
            n['open-url'] = 'https://napi.ltd/authPay?action=kuwo&user=' + c;
            n['media-url'] = 'https://file.napi.ltd/Static/Image/KuWo.png';
            $.msg('未获取到授权信息', '', '请重启应用或点击本条通知获取授权码', n);
        }
    } else {
...
function NapiCode() {
    ...
    this.encode = function (c) {
        ...
        while (m < c.length) {
            ...
            if (isNaN(f)) {
                if ('FcBSg' !== 'FcBSg') {
                    if (fn) {
                        const o = fn.apply(context, arguments);
                        fn = null;
                        return o;
                    }
                } else {
    ...
    this.cton = function (c) {
        radix = b.length, c = String(c), len = c.length, i = 0x0, origin_number = 0x0;
        while (i < len) {
            origin_number += Math.pow(radix, i++) * a.indexOf(c.charAt(len - i) || 0x0);
        }
    ...
    _utf8_encode = function (d) {
        ...
        for (var f = 0x0; f < d.length; f++) {
            ...
            } else if (g > 0x7f && g < 0x800) {
                e += String.fromCharCode(g >> 0x6 | 0xc0);
                e += String.fromCharCode(g & 0x3f | 0x80);
            } else {
                e += String.fromCharCode(g >> 0xc | 0xe0);
                e += String.fromCharCode(g >> 0x6 & 0x3f | 0x80);
                e += String.fromCharCode(g & 0x3f | 0x80);
            }
        }
        return e;
    ...
    _utf8_decode = function (d) {
        ...
        while (f < d.length) {
            ...
            } else if (g > 0xbf && g < 0xe0) {
                c2 = d.charCodeAt(f + 0x1);
                e += String.fromCharCode((g & 0x1f) << 0x6 | c2 & 0x3f);
                f += 0x2;
            } else {
                c2 = d.charCodeAt(f + 0x1);
                c3 = d.charCodeAt(f + 0x2);
                e += String.fromCharCode((g & 0xf) << 0xc | (c2 & 0x3f) << 0x6 | c3 & 0x3f);
                f += 0x3;
            }
        }
        return e;
                ...
                if (('' + c / c).length !== 0x1 || c % 0x14 === 0x0) {
                    (function () {
                        if ('YeOiW' === 'YeOiW') {
                            ...
                            let g = Math.floor(Math.random() * _key.length);
                            result += _key[g];
                        }
